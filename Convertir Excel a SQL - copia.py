...
with open('insertar_datos.sql', 'w', encoding='utf-8') as f:
    ...
    for _, row in cursos_df.iterrows():
        f.write(f"INSERT INTO cursos (nombreCurso, anio, Catedratico, Carrera, Grado) VALUES ('{row['Nombre Curso']}', 2024, {row['Catedrático ID']}, '{row['Carrera']}', '{row['Grado']}');\n")
    f.write("\n")
    ...
    for _, estudiante in estudiantes_df.iterrows():
        ...
        cursos_estudiante = cursos_df[(cursos_df['Grado'] == grado) & (cursos_df['Seccion'] == seccion) & (cursos_df['Carrera'] == carrera)]
        for _, curso in cursos_estudiante.iterrows():
            f.write(f"INSERT INTO estudiantesporcurso (estudiante, curso) VALUES ((SELECT idEstudiante FROM estudiantes WHERE codigoEst = '{estudiante['Código Personal']}'), (SELECT idCurso FROM cursos WHERE nombreCurso = '{curso['Nombre Curso']}' AND Carrera = '{carrera}'));\n")
